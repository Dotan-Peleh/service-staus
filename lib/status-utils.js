...
async function analyzeHtmlFromUrl(url) {
  ...
  // Unity-specific: treat scheduled maintenance as a minor incident
  try {
    const host = new URL(url).hostname || '';
    if (result.state === 'operational' && /(^|\.)status\.unity\.com$/i.test(host)) {
      if (/scheduled\s+maintenance/i.test(plain)) {
        const sentences = plain.split(/(?<=[.!?])\s+/).slice(0, 80);
        const idx = sentences.findIndex(s => /scheduled\s+maintenance/i.test(s));
        const detail = idx >= 0 ? sentences[idx].trim().slice(0, 240) : undefined;
        return { state: 'incident', severity: 'minor', title: 'Scheduled maintenance', detail };
      }
    }
  } catch (_) {}
